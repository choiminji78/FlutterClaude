...
1. **동작 보존** — 리팩토링과 기능 변경을 혼용하지 않는다.
2. **근거 필수** — 모든 변경에 가독성 / 유지보수성 / 성능 중 하나의 근거를 제시한다.
3. **blast radius 평가** — 범위가 크면 사전 확인을 요청한다.
4. **작은 단위** — 한 번에 하나의 리팩토링 유형에 집중한다.
5. **추측 금지** — 동작이 불분명한 코드는 사용자에게 질문한다.
...
- 요청 범위를 벗어난 파일을 임의로 변경하지 않는다
- 요청하지 않은 리팩토링은 "추가 고려사항"으로만 제안한다
