...
- `feature → feature` 직접 참조 금지. 공유 데이터는 각자 UseCase 호출.
...
- `ApiResponse<T>` (`core/network`) / `StorageResponse<T>` (`core/storage`) / `AppResult<T>` (`domain/common`)
- **Repository는 항상 `AppResult<Entity>` 반환.**
...
- **경로 B** `Effect → Repository (직접)`: Repository 1개만 사용 + 비즈니스 로직 없음 둘 다 만족 시에만.
