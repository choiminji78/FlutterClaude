...
import 'load_contact_from_secure_usecase_test.mocks.dart';

@GenerateMocks([ContactRepository])
void main() {
  late MockContactRepository mockRepository;
  late LoadContactFromSecureUseCase useCase;
  ...
  setUp(() {
    ...
    useCase = LoadContactFromSecureUseCase(mockRepository);
  });

  group('LoadContactFromSecureUseCase', () {
    test('repository 성공 시 ContactEntity를 포함한 AppSuccess를 반환한다', () async {
      when(mockRepository.readContactFromSecure())
      ...
      verify(mockRepository.readContactFromSecure()).called(1);
    });
    ...
    test('repository 실패 시 AppFailure를 반환한다', () async {
      when(mockRepository.readContactFromSecure()).thenAnswer(
